use nalgebra::Unit;
use crate::scene::{Vec3, Point, Color, Light, Material, AmbientIllumination, Fog, Texture, hex_to_color};
use crate::ray::{Ray, HitRecord, World};
...
fn apply_grid_texture(texture: &Texture, u: f64, v: f64, base_color: Color) -> Color {
    match texture {
        Texture::Grid { line_color, line_width, cell_size } => {
...
pub fn phong_lighting(
    ...
    let mut color = material.ambient * ambient.intensity * ambient_color.component_mul(&material_color);
    ...
    for light in lights {
        ...
        let shadow_ray = Ray::new(hit_record.point + 0.001 * hit_record.normal.as_ref(), light_dir.as_ref().clone());
        ...
        let diffuse = material.diffuse * diffuse_strength * light.intensity * 
...
pub fn ray_color(
    ...
    if let Some(hit) = world.hit(ray, 0.001, f64::INFINITY) {
        ...
        let material = materials.get(&hit.material_index).cloned().unwrap_or_else(|| Material::default());
        ...
        if let Some(reflectivity) = material.reflectivity {
            if reflectivity > 0.0 && max_depth > 1 {
                ...
                let reflect_ray = Ray::new(hit.point + 0.001 * hit.normal.as_ref(), reflect_dir.as_ref().clone());
                
                let reflected_color = ray_color(
                    &reflect_ray, world, lights, ambient, fog, camera_pos,
                    background_color, materials, max_depth - 1
                );
