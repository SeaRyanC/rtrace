...
impl Transform {
    ...
    pub fn from_str(s: &str) -> Result<Transform, String> {
        ...
        if let Some(params) = s.strip_prefix("rotate(") {
            ...
            let values: Result<Vec<f64>, _> = params
                .split(',')
                .map(|v| v.trim().parse::<f64>())
                .collect();
            match values.map_err(|e| format!("Invalid rotate parameters: {}", e))?.as_slice() {
            ...
            let values: Result<Vec<f64>, _> = params
                ...
                .map(|v| v.trim().parse::<f64>())
                .collect();
            match values.map_err(|e| format!("Invalid translate parameters: {}", e))?.as_slice() {
            ...
            let values: Result<Vec<f64>, _> = params
                ...
                .map(|v| v.trim().parse::<f64>())
                .collect();
            match values.map_err(|e| format!("Invalid scale parameters: {}", e))?.as_slice() {
                [x, y, z] => Ok(Transform::Scale(*x, *y, *z)),
                _ => Err("Scale transform requires exactly 3 parameters (x, y, z)".to_string()),
            }
        } else {
            Err(format!("Unknown transform type. Expected rotate(), translate(), or scale(), got: {}", s))
        }
    }
...
mod tests {
    ...
    fn test_complete_transform_scenario() {
        ...
        assert!((result_point.x - 112.0).abs() < 1e-10, "Expected x=112, got x={}", result_point.x);
        assert!(result_point.y.abs() < 1e-10, "Expected y=0, got y={}", result_point.y); 
        assert!(result_point.z.abs() < 1e-10, "Expected z=0, got z={}", result_point.z);
    }
}
    ...
    pub fn compute_finite_bounds(&self) -> Option<(Point, Point)> {
        ...
        for object in &self.objects {
            let bounds = match object {
                Object::Sphere { center, radius, transform, .. } => {
                ...
                Object::Cube { center, size, transform, .. } => {
                    let mut center_point = Point::new(center[0], center[1], center[2]);
                ...
                Object::Mesh { mesh_data, transform, .. } => {
                        ...
                        if let Some(transform_strings) = transform {
                            if let Ok(transform_matrix) = parse_transforms(transform_strings) {
                                ...
                                let transformed_corners: Vec<Point> = corners
                                    .iter()
                                    .map(|corner| {
                                        let transformed = transform_matrix * corner.to_homogeneous();
