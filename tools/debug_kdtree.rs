...
fn intersect_triangle_moller_trumbore(
    ray: &Ray,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Testing ray: origin=({:.3}, {:.3}, {:.3}), dir=({:.3}, {:.3}, {:.3})",
    ...
    match hit_kdtree.as_ref() {
        Some(h) => println!(
            "  K-d tree: HIT at t={:.6}, point=({:.3}, {:.3}, {:.3})",
    ...
    match hit_brute_force.as_ref() {
        Some(h) => println!(
            "  Brute force: HIT at t={:.6}, point=({:.3}, {:.3}, {:.3})",
            h.t, h.point.x, h.point.y, h.point.z
        ),
        ...
        for (tri_idx, triangle) in mesh.triangles.iter().enumerate() {
            ...
            if let Some((t, _, _)) = intersect_triangle_moller_trumbore(&ray, triangle) {
                if (t - hit.t).abs() < 1e-6 {
                    ...
                    println!(
                        "Triangle bounds: ({:.2}, {:.2}, {:.2}) to ({:.2}, {:.2}, {:.2})",
    ...
    match ray_intersects_overall {
        Some((t_near, t_far)) => {
            ...
            println!(
                "  Intersection range: t_near={:.6}, t_far={:.6}",
                t_near, t_far
            );
